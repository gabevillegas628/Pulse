...
function scoreChip(score: number) {
  const color =
    score >= 1.0 ? 'bg-good-soft text-good border-good/20'
    : score >= 0.5 ? 'bg-warn-soft text-warn border-warn/20'
        ...
        <div className="ml-7 space-y-1">
          ...
          {isWrong && (
            <p className="text-xs text-muted">
              Correct: <span className="text-ink-2 font-medium">{q.correctAnswer}</span>
            </p>
          )}
        </div>
      ) : (
...
export default function SessionGradeSheet({ session, onClose }: Props) {
  ...
  return (
    ...
    >
      <div
        className="w-full md:max-w-lg bg-surface rounded-t-[14px] md:rounded-[14px] max-h-[85vh] flex flex-col border border-hairline"
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-hairline shrink-0">
          ...
          <button onClick={dismiss} className="text-muted hover:text-ink-2 transition-colors">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-6">
          ...
          {!isLoading && !data && (
            <p className="text-sm text-muted text-center py-10">No data available.</p>
          )}
          {data && data.questions.length === 0 && (
            <p className="text-sm text-muted text-center py-10">No questions in this session.</p>
          )}
