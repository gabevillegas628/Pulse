...
const T = {
  panel: {
    ...
    label: '0.6875rem', count: '0.6875rem', note: '0.625rem', dock: '0.625rem',
    gap: 30, labelGap: 46,
  },
  stage: {
    maxStack: 300, minR: 3, maxR: 9,
    label: 'clamp(10px, 1.35vw, 23px)',
    count: 'clamp(10px, 1.35vw, 23px)',
...
export default function NumericDots({
  ...
  const r = Math.max(t.minR, Math.min(t.maxR, t.maxStack / (2 * tallest), step > 0 ? step * 0.44 : t.maxR))

  const stackH = Math.min(t.maxStack, tallest * 2 * r)
  const H = stackH + t.gap + 26
  const base = H - t.gap
  ...
  const EDGE = variant === 'stage' ? 46 : 26

  /** Gap between dot centres. Tightens as a stack grows so the top dot still lands inside. */
  const spacing = (n: number) => (n > 1 ? Math.min(2 * r, (stackH - 2 * r) / (n - 1)) : 0)
    ...
    <div ref={boxRef}>
      <svg width={W} height={H} style={{ display: 'block', overflow: 'visible' }} role="img"
        ...
        {docked > 0 && (
          <g>
            ...
            {Array.from({ length: docked }, (_, i) => (
              <circle key={i} cx={2 + dockW / 2} cy={base - r - i * Math.min(r * 2.4, spacing(docked))}
        ...
        {cols.map((c, i) => {
          ...
          return (
            <g key={i}>
              {/* Spacing tightens rather than the column being truncated: a stack taller
                  than the panel packs its dots closer instead of dropping the ones that do
                  not fit, so the count on screen is always the count of responses. */}
              {Array.from({ length: c.count }, (_, k) => (
                <circle key={k} cx={x} cy={base - r - k * dy} r={r * 0.85} fill={fill} opacity={0.9} />
              ))}
              {labelled.has(i) && (
                <>
                  <text x={lx} y={base - 2 * r - (c.count - 1) * dy - 6} textAnchor={anchor}
                  ...
                  <text x={lx} y={base + 22} textAnchor={anchor}
